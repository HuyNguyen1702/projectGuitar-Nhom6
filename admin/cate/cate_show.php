...
<body>
    ...
    if ($search) {
        $where = "WHERE 'cate_id' LIKE '%" . $search . "% OR 'cate_name' LIKE  '%" . $search . "%' ";
    }
    $item_per_page = !empty($_GET['per_page']) ? $_GET['per_page'] : 4;
    $current_page = !empty($_GET['page']) ? $_GET['page'] : 1;
    $offset = ($current_page - 1) * $item_per_page;
    if ($search) {
        $sql = "SELECT * FROM `tbl_category` WHERE `cate_id` LIKE '%" . $search . "%' OR `cate_name` LIKE '%" . $search . "%' LIMIT " . $item_per_page . " OFFSET " . $offset;
        $totalRecords = mysqli_query($connect, "SELECT * FROM `tbl_category` WHERE `cate_id` LIKE '%" . $search . "%' OR `cate_name` LIKE '%" . $search . "%' ");
    } else {
        $sql = "SELECT * FROM tbl_category LIMIT " . $item_per_page . " OFFSET " . $offset;
        $totalRecords = mysqli_query($connect, "SELECT * FROM `tbl_category`");
    }
            ...
            <div class="display-data">
                ...
                <div class="action" style="margin-bottom: 20px;">
                    <form action="" method="GET">
                        ...
                        <a class="btn btn-primary" href="cate.php?page_layout=add">Thêm mới</a>
                        <a href="cate.php?page_layout=show" class="btn btn-primary"><i class="fa fa-refresh"></i>Tải lại</a>
                    </form>
                </div>
                    ...
                    <table style="width: 1640px; border-collapse: inherit;  border: 1px solid #ddd; border-radius: 20px;">
                        ...
                        <?php while ($row = mysqli_fetch_assoc($query)) : ?>
                            <tr>
                                ...
                                <td>
                                    <div class="pro_id">
                                        <?php echo $row['cate_status'] ?>
                                    </div>
                                </td>
                                <td style="padding: 20px !important;">
                                    <div class="button-edit">
                                        <a type="button" href="cate.php?page_layout=edit&cate_id=<?php echo $row['cate_id']; ?>">Edit</a>
                                    </div>
                                </td>
                                <td style="padding: 10px !important;">
                                    <div class="btn-del">
                                        <a type="button" href="cate.php?page_layout=delete&cate_id=<?php echo $row['cate_id']; ?>">Delete</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endwhile; ?>
